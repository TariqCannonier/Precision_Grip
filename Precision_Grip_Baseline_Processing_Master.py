...
#     working_data.plot_sensors(kind='topomap',show_names=True)
#     emg_only.plot_sensors(kind='topomap',show_names=True) # emg electrodes are spatially skewed from the raw montage
    mne.set_bipolar_reference(emg_only,['T7' , 'PO7'], ['T8' , 'PO8'],                               ch_name=['T7-T8' , 'PO7-PO8'],drop_refs=False,copy=False)
#     emg_only.plot_sensors(kind='topomap',show_names=True) # the virtual emg electrodes are overlayed by the original electrode
    
    ## set EEG bipolar reference
#     eeg_only.plot_sensors(kind='topomap',show_names=True) # eeg electrodes are spatially skewed from the raw montage
    mne.set_bipolar_reference(eeg_only,['C3' , 'C4'], ['Cz' , 'Cz'],                              ch_name=['C3-Cz' , 'C4-Cz'],drop_refs=False,copy=False)
#     eeg_only.plot_sensors(kind='topomap',show_names=True) # Can see that the bipolar reference shifts the other electrodes to the left
...
if print_diagnostic==True: # test function
    ...
    plot_channels([(working_data.copy().pick_types(eeg=True,emg=False)) , 
                   filtered_data['eeg'] , filtered_data['emg'] ])
...
def crop_data (data_file, timestamps, print_diagnostic = False ):
    ...
    for i in range(0,len(timestamps)):
        ...
        blocks_data.append(data_file.copy().crop                            (tmin=tmin,tmax=tmax)) # return a list of eeg lab arrays split into blocks by timestamps
        
        blocks_events.append(mne.make_fixed_length_events                              (blocks_data[i],id=id_label,duration=event_duration)) # for each block, return a list of arrays with event markers 0,1 every 2s

        # need to rename this to be event marked_block or marked_data; blocks_epochs is not accurate
        blocks_epochs.append(mne.Epochs                              (blocks_data[i],blocks_events[i], event_id=id_label,                               tmin=0,tmax=2, baseline=None,                               preload=True,verbose=False)) # add 2s [0,1] event markers to each block in array
...
if test_fxn==True: # test function
    # leaving out EMG processing until we know how we want to crop emg data
    blocks_data, blocks_events, blocks_epochs = crop_data(filtered_data['eeg'] , epochs, print_diagnostic)
...
if test_fxn==True: # test function
    ...
    block_figs = generate_block_figs(blocks_data[0], blocks_events[0],                                           blocks_epochs[0], n_epochs, duration, scalings);
...
def reporting(epochs, subject_ID, blocks_data, blocks_events, blocks_epochs,              working_data, eeg_filtered, events, report_directory):
    ...
    rep.add_figs_to_section(partic_figs, captions=["raw psd","filtered psd","events"],                            section="Subject "+subject_ID)
    
    for i in range(0,len(epochs)): # loop through blocks
        
        # make figures
        block_figs = generate_block_figs(blocks_data[i], blocks_events[i],                                         blocks_epochs[i], n_epochs, duration, scalings);
        
        # define figure captions
        captions = ['Block %d Data' % (i+1),                     'Block %d Butterfly' % (i+1), 'Block %d Topomap' % (i+1),'Block %d TopoJoint' % (i+1)]
...
if test_report==True: # test function
    ...
    # leaving out EMG processing until we know how we want to crop emg data
    reporting(epochs, participant_ID, blocks_data, blocks_events, blocks_epochs, working_data,              filtered_data['eeg'], events, report_directory)
...
def run_reports( subject_list, filter_params, n_epochs, duration, scalings ):
    ...
    for e in subject_list: # loop through subjects, set input path
        ...
        if e in participant_info.keys():
            ...
            # leaving out EMG processing until we know how we want to crop emg data
            blocks_data, blocks_events, blocks_epochs = crop_data(filtered_data['eeg'] , epochs, False)

            reporting(epochs, subject_ID, blocks_data, blocks_events, blocks_epochs,                     working_data, filtered_data['eeg'], events, report_directory);
